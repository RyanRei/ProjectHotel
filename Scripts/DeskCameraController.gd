...
@export_range(0.1, 5.0, 0.1) var horizontal_move_limit := 2 # Left / Right distance limit
@export_range(0.1, 5.0, 0.1) var forward_move_limit := 0.5    # Forward distance limit
@export_range(0.1, 5.0, 0.1) var backward_move_limit := 0.6  # Backward distance limit
...
func _ready() -> void:
	...
	# Lock in your starting coordinates. base_height acts as your Y-level floor lock.
...
func _process(delta: float) -> void:
	...
	# --- ASYMMETRIC CLAMPING ---
	# Left / Right limit (-X to +X)
	target_xz_offset.x = clamp(
		target_xz_offset.x,
		-horizontal_move_limit,
		horizontal_move_limit
	)
	# Forward (-Z) / Backward (+Z) limits
	...
	current_xz_offset = current_xz_offset.lerp(target_xz_offset, 1.0 - exp(-position_smoothing * delta))
